...
formulario.addEventListener("submit", (e)=>{
    ...
    let expresionFechaNacimiento = /^(0[1-9]|[1-2][0-9]|3[0-1])\/(0[1-9]|1[0-2])\/\d{4}$/;  //--> esta NO la uso pero si la queremos usar, debemos hacer un input de tipo text y NO de tipo date
    ...
    if (contrasenia.value == "") {
        mensaje += "<label for='register__password'>* Introduce una CONTRASEÑA</label>"
        ...
        if (expresionContrasenia.test(contrasenia.value) == false) {
            mensaje += "<label for='register__password'>* Contraseña  inválida</label>";
            contraseniaValidada = false;
        }
    }
...
*/
    ...
    if (contrasenia.value != "" && repetirContrasenia.value != "") {
        ...
        if (expresionContrasenia.test(contrasenia.value) == true && expresionContrasenia.test(repetirContrasenia.value) == true) {
            
            // ahora, si los valores de las dos, son distintos, entra al 3er if
            if (contrasenia.value != repetirContrasenia.value) {
                mensaje += "<label for='repit__password'>* NO COINCIDEN LAS CONTRASEÑAS</label>";
...
if (!nombreValidado || !contraseniaValidada || !repetirContraseniaValidada || !fechaNacimientoValidada || !emailValidado || !lasContraseniasCoinciden) {
